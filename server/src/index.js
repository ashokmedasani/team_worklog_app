...
async function logActivity({ projectId = null, actor = 'Unknown', action, entityType, entityId = null, summary, details = null }) {
  await db.prepare(`
...
app.post('/api/users', async (req, res) => {
  ...
  const user = await db.prepare('SELECT id, username, display_name, role, active, created_at, updated_at FROM users ORDER BY id DESC LIMIT 1').get();
  await logActivity({ actor: actorName(req), action: 'created', entityType: 'User', entityId: user.id, summary: `Created login user "${user.username}" as ${user.role}`, details: user });
...
app.put('/api/users/:id', async (req, res) => {
  ...
  const existing = (await db.prepare('SELECT * FROM users').all()).find((row) => Number(row.id) === id);
  ...
  const user = await db.prepare('SELECT id, username, display_name, role, active, created_at, updated_at FROM users WHERE id = ?').get(id);
  await logActivity({ actor: actorName(req), action: 'updated', entityType: 'User', entityId: user.id, summary: `Updated login user "${user.username}"`, details: user });
  res.json(user);
});
...
app.delete('/api/users/:id', async (req, res) => {
  ...
  const user = (await db.prepare('SELECT id, username, display_name, role, active, created_at, updated_at FROM users').all()).find((row) => Number(row.id) === id);
  if (!user) return res.status(404).json({ error: 'User not found.' });
  await db.exec(`DELETE FROM users WHERE id = ${id}`);
  await logActivity({ actor: actorName(req), action: 'deleted', entityType: 'User', entityId: user.id, summary: `Deleted login user "${user.username}"`, details: user });
  res.status(204).end();
});

app.get('/api/projects', async (_req, res) => {
  res.json(await db.prepare('SELECT * FROM projects ORDER BY updated_at DESC').all());
});
...
app.post('/api/projects', async (req, res) => {
  ...
  const project = await db.prepare('SELECT * FROM projects ORDER BY id DESC LIMIT 1').get();
  await logActivity({ projectId: project.id, actor: actorName(req), action: 'created', entityType: 'Project', entityId: project.id, summary: `Created project "${project.name}"`, details: project });
...
app.put('/api/projects/:id', async (req, res) => {
  ...
  await logActivity({ projectId: project.id, actor: actorName(req), action: 'updated', entityType: 'Project', entityId: project.id, summary: `Updated project "${project.name}"`, details: project });
  res.json(project);
});
...
app.delete('/api/projects/:id', async (req, res) => {
  if (!requireAdmin(req, res)) return;
  const id = Number(req.params.id);
  const project = (await db.prepare('SELECT * FROM projects').all()).find((row) => Number(row.id) === id);
  if (!project) return res.status(404).json({ error: 'Project not found.' });
  const snapshot = {
    project,
    tasks: (await db.prepare('SELECT * FROM tasks').all()).filter((row) => Number(row.project_id) === id),
    members: (await db.prepare('SELECT * FROM project_members').all()).filter((row) => Number(row.project_id) === id)
  };
  await db.exec(`DELETE FROM projects WHERE id = ${id}`);
  await logActivity({ projectId: null, actor: actorName(req), action: 'deleted', entityType: 'Project', entityId: Number(req.params.id), summary: `Deleted project "${project.name}"`, details: snapshot });
  res.status(204).end();
});
...
app.post('/api/project-members', async (req, res) => {
  ...
  const member = await db.prepare('SELECT * FROM project_members ORDER BY id DESC LIMIT 1').get();
  await logActivity({ projectId: member.project_id, actor: actorName(req), action: 'created', entityType: 'Team Member', entityId: member.id, summary: `Added ${member.name} as ${member.access_level}`, details: member });
  res.status(201).json(member);
});

app.put('/api/project-members/:id', async (req, res) => {
  if (!requireAdmin(req, res)) return;
  const { project_id, name, role = '', access_level = 'Employee', email = '', phone = '', notes = '' } = req.body;
  const result = await db.prepare(`
  ...
  const member = await db.prepare('SELECT * FROM project_members WHERE id = ?').get(req.params.id);
  await logActivity({ projectId: member.project_id, actor: actorName(req), action: 'updated', entityType: 'Team Member', entityId: member.id, summary: `Updated ${member.name}`, details: member });
  res.json(member);
});
...
app.delete('/api/project-members/:id', async (req, res) => {
  if (!requireAdmin(req, res)) return;
  const id = Number(req.params.id);
  const member = (await db.prepare('SELECT * FROM project_members').all()).find((row) => Number(row.id) === id);
  if (!member) return res.status(404).json({ error: 'Team member not found.' });
  await db.exec(`DELETE FROM project_members WHERE id = ${id}`);
  await logActivity({ projectId: member.project_id, actor: actorName(req), action: 'deleted', entityType: 'Team Member', entityId: member.id, summary: `Deleted team member "${member.name}"`, details: member });
  res.status(204).end();
});
...
app.get('/api/dashboard', async (req, res) => {
  ...
  const hoursByMember = (await db.prepare(`
...
app.post('/api/tasks', async (req, res) => {
  ...
  const task = await db.prepare('SELECT * FROM tasks ORDER BY id DESC LIMIT 1').get();
  await logActivity({ projectId: task.project_id, actor: actorName(req), action: 'created', entityType: 'Task', entityId: task.id, summary: `Created task "${task.title}"`, details: task });
...
app.put('/api/tasks/:id', async (req, res) => {
  ...
  await logActivity({ projectId: task.project_id, actor: actorName(req), action: 'updated', entityType: 'Task', entityId: task.id, summary: `Updated task "${task.title}"`, details: task });
  res.json(task);
});
...
app.delete('/api/tasks/:id', async (req, res) => {
  ...
  const id = Number(req.params.id);
  const task = (await db.prepare('SELECT * FROM tasks').all()).find((row) => Number(row.id) === id);
  if (!task) return res.status(404).json({ error: 'Task not found.' });
  const snapshot = {
    task,
    workLogs: (await db.prepare('SELECT * FROM work_logs').all()).filter((row) => Number(row.task_id) === id),
    notes: (await db.prepare('SELECT * FROM notes').all()).filter((row) => Number(row.task_id) === id)
  };
  await db.exec(`DELETE FROM tasks WHERE id = ${id}`);
  await logActivity({ projectId: task.project_id, actor: actorName(req), action: 'deleted', entityType: 'Task', entityId: task.id, summary: `Deleted task "${task.title}"`, details: snapshot });
  res.status(204).end();
});
...
app.post('/api/work-logs', async (req, res) => {
  ...
  const log = await db.prepare('SELECT * FROM work_logs ORDER BY id DESC LIMIT 1').get();
  const task = await db.prepare('SELECT project_id, title FROM tasks WHERE id = ?').get(log.task_id);
  await logActivity({ projectId: task?.project_id, actor: actorName(req), action: 'created', entityType: 'Work Log', entityId: log.id, summary: `Logged work on "${task?.title || 'Task'}"`, details: log });
...
app.put('/api/work-logs/:id', async (req, res) => {
  ...
  const log = await db.prepare('SELECT * FROM work_logs WHERE id = ?').get(req.params.id);
  const task = await db.prepare('SELECT project_id, title FROM tasks WHERE id = ?').get(log.task_id);
  await logActivity({ projectId: task?.project_id, actor: actorName(req), action: 'updated', entityType: 'Work Log', entityId: log.id, summary: `Updated work log on "${task?.title || 'Task'}"`, details: log });
  res.json(log);
});
...
app.delete('/api/work-logs/:id', async (req, res) => {
  if (!requireManager(req, res)) return;
  const id = Number(req.params.id);
  const log = (await db.prepare('SELECT * FROM work_logs').all()).find((row) => Number(row.id) === id);
  if (!log) return res.status(404).json({ error: 'Work log not found.' });
  const task = await db.prepare('SELECT project_id, title FROM tasks WHERE id = ?').get(log.task_id);
  await db.exec(`DELETE FROM work_logs WHERE id = ${id}`);
  await logActivity({ projectId: task?.project_id, actor: actorName(req), action: 'deleted', entityType: 'Work Log', entityId: log.id, summary: `Deleted work log on "${task?.title || 'Task'}"`, details: log });
  res.status(204).end();
});
...
app.post('/api/notes', async (req, res) => {
  ...
  await db.prepare('INSERT INTO notes (task_id, author, content, note_date) VALUES (?, ?, ?, ?)').run(task_id, author, content, note_date);
  const note = await db.prepare('SELECT * FROM notes ORDER BY id DESC LIMIT 1').get();
  const task = await db.prepare('SELECT project_id, title FROM tasks WHERE id = ?').get(note.task_id);
  await logActivity({ projectId: task?.project_id, actor: actorName(req), action: 'created', entityType: 'Note', entityId: note.id, summary: `Added note on "${task?.title || 'Task'}"`, details: note });
...
app.put('/api/notes/:id', async (req, res) => {
  ...
  const task = await db.prepare('SELECT project_id, title FROM tasks WHERE id = ?').get(note.task_id);
  await logActivity({ projectId: task?.project_id, actor: actorName(req), action: 'updated', entityType: 'Note', entityId: note.id, summary: `Updated note on "${task?.title || 'Task'}"`, details: note });
  res.json(note);
});
...
app.delete('/api/notes/:id', async (req, res) => {
  if (!requireManager(req, res)) return;
  const id = Number(req.params.id);
  const note = (await db.prepare('SELECT * FROM notes').all()).find((row) => Number(row.id) === id);
  if (!note) return res.status(404).json({ error: 'Note not found.' });
  const task = await db.prepare('SELECT project_id, title FROM tasks WHERE id = ?').get(note.task_id);
  await db.exec(`DELETE FROM notes WHERE id = ${id}`);
  await logActivity({ projectId: task?.project_id, actor: actorName(req), action: 'deleted', entityType: 'Note', entityId: note.id, summary: `Deleted note on "${task?.title || 'Task'}"`, details: note });
  res.status(204).end();
});

app.get('/api/export/tasks.csv', async (_req, res) => {
  const rows = await db.prepare(`
...
app.get('/api/export/work-logs.csv', async (_req, res) => {
  const rows = (await db.prepare(`
